...
from models.hivt_gan import HiVTGAN  
...
def main():
    ...
    # Route to the GAN Model
    if args.train_gan:
        print("--- initializing HiVT_GAN ---")
        # CRITICAL FIX: Removed args.num_modes = 1 so it defaults to 6
        model = HiVTGAN(**vars(args))
    else: 
        print(f"--- initializing Standard HiVT ---") 
    ...
    if args.ckpt_path:
        ...
        has_critics = any("critics" in k for k in ckpt['state_dict'].keys())
